...
public class SpawnLoader implements Listener {
     ...
     */
    private static final int MAX_SEARCH_WIDTH = 50;

    /**
     * The plugin used to register events and timers
     */
    ...
    /**
     * A map containing all bukkit tasks associated with these threads
     */
    private Map<Integer, BukkitTask> threadTasks;
    /**
    ...
    public SpawnLoader(JavaPlugin plugin, WorldManager worldManager, List<Location> locations, Runnable onLoad) {
        this.plugin = plugin;
    ...
    public void start() {
        ...
        for (int i = 0; i < this.locations.size(); i++) {
            ...
            this.threadTasks.put(i, bukkitTask);
        }
    ...
    private void checkThreads() {
        for (int i = 0; i < this.findThreads.size(); i++) {
            ...
            if (spawnFindThread.isDone()) {
                ...
                this.threadTasks.get(i).cancel();

                // Remove from lists
                this.findThreads.remove(i);
                this.threadTasks.remove(i);
